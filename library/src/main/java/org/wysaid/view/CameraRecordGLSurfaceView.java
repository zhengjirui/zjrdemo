...
public class CameraRecordGLSurfaceView extends CameraGLSurfaceView {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                if (mFrameRecorder == null) {
                    Log.e("kwan", "Error: startRecording after release!!");
                ...
                if (!mFrameRecorder.startRecording(30, filename)) {
                    Log.e("kwan", "start recording failed!");
                    if (recordingCallback != null)
                        recordingCallback.startRecordingOver(false);
                    return;
                }
                Log.e("kwan", "glSurfaceView recording, file: " + filename);
    ...
    public void endRecording(final EndRecordingCallback callback, final boolean shouldSave) {
        Log.e("kwan", "notify quit...");
        synchronized (mRecordStateLock) {
            mShouldRecord = false;
        }

        if (mFrameRecorder == null) {
            Log.e("kwan", "Error: endRecording after release!!");
            return;
        }

        joinAudioRecording();
		Log.e("kwan", "joinAudioRecording over ...");
        queueEvent(new Runnable() {
            @Override
            public void run() {
                if (mFrameRecorder != null)
    ...
    public void joinAudioRecording() {
        ...
        if (mAudioThread != null) {
			...
			Log.e("kwan", "joinAudioRecording over ...1");

            try {
				Log.e("kwan", "joinAudioRecording over ...1.1");
                mAudioThread.join();
				Log.e("kwan", "joinAudioRecording over ...1.2");
                mAudioThread = null;
            } catch (Exception e) {
                e.printStackTrace();
				Log.e("kwan", "joinAudioRecording over ...1.3"+e.getMessage());
				Log.e("kwan", "joinAudioRecording over ...1.3");
            }
        }

		Log.e("kwan", "joinAudioRecording over ...2");
    }
    ...
    class AudioRecordRunnable implements Runnable {
        ...
        private AudioRecordRunnable(StartRecordingCallback callback) {
            ...
            try {
                ...
                Log.e(LOG_TAG, "audio min buffer size: " + bufferSize);
...
//                audioData = new short[bufferSize];
				...
				Log.e("kwan", "audio min buffer size: " + e.getMessage());
